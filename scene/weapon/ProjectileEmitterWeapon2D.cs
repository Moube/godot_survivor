...
public partial class ProjectileEmitterWeapon2D : Weapon2D
{
	...
	[Export]
	public ProjectileFireMode FireMode { get; set; } = ProjectileFireMode.MouseDirection;
	...
	{
		return config.BehaviorType == WeaponBehaviorType.ProjectileEmitter;
	}
	...
	protected virtual void OnEmitterUpdate(double delta)
	{
	}
	...
	private void UpdateFireAnimation(double delta)
	{
		...
		{
			if (_fireCooldownRemaining <= 0.0 && TryResolveFireDirection(out Vector2 fireDirection))
			{
				_currentFireDirection = fireDirection;
				StartFireAnimation();
			}
			else if (Sprite != null)
			{
	...
	private void UpdateVisualAim()
	{
		...
		if (FireMode == ProjectileFireMode.RandomDirection)
		{
			return;
		}

		if (TryResolveFireDirection(out Vector2 fireDirection))
		{
			GlobalRotation = fireDirection.Angle();
		}
	}

	private bool TryResolveFireDirection(out Vector2 direction)
	{
